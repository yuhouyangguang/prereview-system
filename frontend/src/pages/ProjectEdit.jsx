...
export default function ProjectEdit() {
  ...
  useEffect(() => {
    api.get(`/projects/${id}`).then(r => {
      const p = r.data
      setForm({
        client_name: p.client_name || '', client_industry: p.client_industry || '',
        client_credit_rating: p.client_credit_rating || '', client_description: p.client_description || '',
        loan_amount: p.loan_amount || '', loan_purpose: p.loan_purpose || '',
        loan_type: p.loan_type || '', loan_term: p.loan_term || '', guarantee_type: p.guarantee_type || '',
        interest_rate: p.interest_rate || '', fee_rate: p.fee_rate || '',
        deposit_return: p.deposit_return || '', competitor_rate: p.competitor_rate || '',
    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1>编辑项目</h1>
            <p style={{ marginTop: 4 }}>修改项目基本信息</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="form-group">
            <label>客户简介</label>
            <textarea className="form-control" value={form.client_description} onChange={e => set('client_description', e.target.value)} rows={3} />
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="grid-3">
            <div className="form-group">
              <label>融资金额（万元）</label>
              <input type="number" className="form-control" value={form.loan_amount} onChange={e => set('loan_amount', e.target.value)} />
            </div>
            <div className="form-group">
              <label>贷款类型</label>
            ...
            <div className="form-group">
              <label>贷款期限（月）</label>
              <input type="number" className="form-control" value={form.loan_term} onChange={e => set('loan_term', e.target.value)} />
            </div>
          </div>
          <div className="form-group">
            <label>融资用途</label>
            <input className="form-control" value={form.loan_purpose} onChange={e => set('loan_purpose', e.target.value)} />
          </div>
          <div className="form-group">
            <label>担保方式</label>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="grid-3">
            <div className="form-group">
              <label>贷款利率（%）</label>
              <input type="number" step="0.01" className="form-control" value={form.interest_rate} onChange={e => set('interest_rate', e.target.value)} />
            </div>
            <div className="form-group">
              <label>手续费率（%）</label>
              <input type="number" step="0.01" className="form-control" value={form.fee_rate} onChange={e => set('fee_rate', e.target.value)} />
            </div>
            <div className="form-group">
              <label>存款回报（%）</label>
              <input type="number" step="0.01" className="form-control" value={form.deposit_return} onChange={e => set('deposit_return', e.target.value)} />
            </div>
            <div className="form-group">
              <label>同业参考利率（%）</label>
              <input type="number" step="0.01" className="form-control" value={form.competitor_rate} onChange={e => set('competitor_rate', e.target.value)} />
            </div>
            <div className="form-group">
              <label>参考银行</label>
              <input className="form-control" value={form.competitor_bank} onChange={e => set('competitor_bank', e.target.value)} />
            </div>
          </div>
        </div>

        <div className="flex-end gap-3">
          <button type="button" onClick={() => navigate(`/projects/${id}`)} className="btn btn-ghost">取消</button>
